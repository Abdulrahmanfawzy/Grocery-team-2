...
import { Search, X, ShoppingCart, Trash2, Plus } from "lucide-react";
...
import type { ProductFilters, Product } from "@/features/products/types/product.types";
...
export default function ProductList() {
  ...
  // State لإدارة كمية كل منتج بشكل منفصل
  const [quantities, setQuantities] = useState<Record<number | string, number>>({});

  const products = data?.data ?? [];

  const handleQuantityChange = (productId: number | string, delta: number) => {
    setQuantities((prev) => {
  ...
  const handleFilterChange = (nextFilters: ProductFilters) => {
    ...
    if (nextFilters.category !== undefined) {
      newParams.set("category", String(nextFilters.category));
    }
    ...
    return (
      <div className="flex min-h-[400px] items-center justify-center px-4">
        <div className="w-full max-w-md rounded-2xl border border-red-100 bg-red-50 p-8 text-center">
          <h2 className="text-xl font-bold text-red-600">No Products Found</h2>
          <Link to={"/ProductList"} onClick={handleClearSearch} className="mt-2 inline-flex items-center gap-1 text-sm text-red-600 underline">
            <X className="h-4 w-4" />
            Clear Search
          </Link>
              ...
              products.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* ==================== Add To Cart & Quantity (Figma Design) ==================== */}
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={isPending}
                        className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[#08415F] py-2.5 px-2 text-xs sm:text-sm font-semibold text-white transition hover:bg-[#01304a] disabled:cursor-not-allowed disabled:opacity-50"
                      >
                      ...
                      {/* Quantity Controls */}
                      <div className="flex items-center justify-between rounded-lg border border-gray-300 px-2 py-1.5">
                        <button
                          type="button"
