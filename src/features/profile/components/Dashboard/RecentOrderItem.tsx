...
const RecentOrderItem=({orderNumber,date,itemsCount,status,price}:IProps)=> {
  return (
      <div className="mt-3 flex w-full items-center justify-between gap-3 border-b border-[#D1D5DC] py-4 last:border-b-0">
        ...
        <div className="flex items-start justify-between flex-col gap-2">
              ...
              <p className="break-words text-[14px] leading-[120%] text-[#0A0A0A] sm:text-[16px]">{date}</p>
              <p className="text-[#4A5565] text-[12px] leading-[150%]">{itemsCount} items</p>
            
        </div>
       {/* status & price */}
        <div className="shrink-0 flex flex-col items-start justify-between gap-2">
